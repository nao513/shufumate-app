...
def create_plan_for_date(client, date_str, gender, age, weight, body_fat, target_weight):
    prompt = f"""
...
# -----------------------------
    ...
    with col1:
        loan_price = st.number_input("住宅購入金額（万円）", value=4000)
        loan_rate = st.slider("住宅ローン金利（%）", 0.1, 5.0, 1.0)
        loan_years = st.slider("返済年数", 10, 40, 35)
        monthly_loan = (loan_price * 10000 * (loan_rate / 100 / 12)) / (1 - (1 + loan_rate / 100 / 12) ** (-loan_years * 12))
        loan_total = monthly_loan * 12 * loan_years / 10000
        st.metric("月々の住宅ローン", f"{monthly_loan:,.0f} 円")
        st.metric("総支払額", f"{loan_total:,.0f} 万円")

    with col2:
        rent = st.number_input("毎月の家賃（円）", value=90000)
        rent_years = st.slider("賃貸予定年数", 5, 50, 35)
        rent_total = rent * 12 * rent_years / 10000
        st.metric("賃貸総額", f"{rent_total:,.0f} 万円")

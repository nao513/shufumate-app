...
# --- 設定 ---
    ...
    weight = st.number_input("体重", 30.0, 200.0, 60.0)
    target_weight = st.number_input("目標体重", 30.0, 200.0, 55.0)
    body_fat = st.number_input("体脂肪率", 5.0, 60.0, 28.0)

    days = st.slider("何日分？", 1, 30, 7)

    api_key = st.text_input("OpenAI APIキー", type="password")

    if st.button("作成する") and api_key:
...
"""
            ...
            results.append({
                "日付": date,
                "内容": res.choices[0].message.content
            })
